...
export function EditProyectoForm({
  proyecto: o,
  clientes,
  trabajadores,
}: Props) {
  ...
  const [contactos, setContactos] = useState<ContactoCliente[]>(
    o.coordinadorCliente
      ? [
          {
            id: o.coordinadorCliente.id,
            nombre: o.coordinadorCliente.nombre,
            cargo: o.coordinadorCliente.cargo,
            activo: true,
            clienteId: o.clienteId ?? "",
            creadoEn: "",
          },
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-8 bg-white">
          ...
          <div>
            ...
            >
              <SelectTrigger className="w-full">
                <SelectValue
                  placeholder={
                    form.clienteId ? "Sin asignar" : "Sin cliente seleccionado"
                  }
                />
              </SelectTrigger>
              <SelectContent>
      ...
      {/* Turnos de horario */}
      <section className="space-y-4">
        <h2 className={sectionTitleCn}>Turnos de horario</h2>
        <p className="text-xs text-muted-foreground">
          Define uno o más turnos (mañana, tarde, noche) para tomar asistencia.
          Los cambios se guardan de inmediato, no requieren &quot;Guardar
          cambios&quot;.
        </p>
        <TurnoConfigListEditor
          proyectoId={o.id}
          initial={o.turnoConfigs ?? []}
        />
      </section>
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            />
            {errors.fechaFin && (
              <p className="mt-1 text-xs text-destructive">{errors.fechaFin}</p>
            )}
          </div>
          <div>
            <label className={labelCn}>
