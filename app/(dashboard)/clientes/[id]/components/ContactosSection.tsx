...
import type { ContactoCliente } from '@/types/api'
...
export function ContactosSection({ clienteId, contactos }: Props) {
  ...
  const [editTarget, setEditTarget] = useState<ContactoCliente | null>(null)

  async function handleCreate(form: FormData) {
    const payload = Object.fromEntries(
      Object.entries(form).filter(([, v]) => v !== ''),
    )
    await api.post(`/clientes/${clienteId}/contactos`, payload)
    setCreateOpen(false)
    router.refresh()
  }
  ...
  async function handleEdit(form: FormData) {
    ...
    await api.patch(`/clientes/${clienteId}/contactos/${editTarget.id}`, payload)
    setEditTarget(null)
    router.refresh()
  }
            ...
            {contactos.map((contacto) => (
              <div key={contacto.id} className="flex items-start justify-between py-3 border rounded-lg p-2">
                ...
                <button
                  ...
                  className="text-xs text-muted-foreground underline-offset-2 hover:underline hover:text-foreground transition-colors duration-[120ms] shrink-0 ml-4"
                >
                  Editar
                </button>
              </div>
